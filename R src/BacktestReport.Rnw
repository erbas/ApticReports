...
\begin{wide}
\section{\Huge \Sexpr{strategy} : \Sexpr{ccy.pair} : \Sexpr{timeframe} : \Sexpr{strat.dir}}
...
SHARPE <- SharpeRatio.annualized(pnl.daily2,Rf=0,scale=252,geometric=T)
...
WINLOSS <- apply(pnl.raw,2,function(x) length(x[x>0])/length(x)*100)
...
statstable <- data.frame(rbind(ABSRTN,CAR,MAXDRAW,RECOV,CONSEC,VOL,SHARPE,SORTINO,ABSRTN.10,ABSRTN.11,ABSRTN.12,ABSRTN.13))
...
plot(cumsum(pnl.london2)*100,main="Returns from London Trades",ylab="% AUM")
plot(cumsum(pnl.newyork2)*100,main="Returns from New York Trades",ylab="% AUM")
plot(cumsum(pnl.asia2)*100,main="Returns from Asia Trades",ylab="% AUM")
@
\end{figure}
\end{textblock*}
