...
require(xtable)
require(corrplot)
source("MyChartsPerformanceSummary.R")
...
statstable <- data.frame(rbind(ABSRTN,
                               CAR,
                               MAXDRAW,
                               #RECOV,
                               VOL,
...
layout(matrix(1:6,1,2),respect=FALSE)
cex.m <- 1
cex.lg <- 0.7
op <-par(cex.main=0.7, cex.axis=0.7, mex=0.7)
...
plot.zoo(vols[,1],plot.type='single',col=rich6equal,main=main.txt,xlab='',ylab='',lwd=2)
...
plot.zoo(corln,plot.type='single',col=rich6equal,main=main.txt,xlab='',ylab='',lwd=2)
