...
load_and_process <- function(filenames, index.path, output_path, AUM, report_name, start.end.dates, rel_returns, ptf.of.ptf) {
  # filenames is character vector or list of full paths
  ...
  new.edge.xts <- xts(vals, mdy(new.edge.csv[,1],tz="Europe/London"))
...
do.knitting <- function(filestem, output_path) {
